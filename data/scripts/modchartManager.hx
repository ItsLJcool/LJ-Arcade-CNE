...
var modchart_initialized = false;
...
public function initModchart()
{
	...
	// trace("modShaderVertTable: " + modShaderVertTable);
...
var _modifierTweens:Map<String, FlxTween> = [];
public function tweenModifierValue(name:String, newValue:Float, time:Float, easeFunc:Float->Float)
{
	...
	var easeFunc = easeFunc;
	if (_modifierTweens[name] != null) _modifierTweens[name].cancel();
	var tween = FlxTween.num(startValue, newValue, time, {onUpdate: function(tween:FlxTween){
